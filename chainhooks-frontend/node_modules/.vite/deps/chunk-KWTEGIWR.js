...
var featureConfig = {
  ...
  socials: {
    ...
    processApi: (apiConfig) => {
      ...
      const config3 = apiConfig.config;
      return Boolean(apiConfig.isEnabled) && config3.length > 0 ? config3.filter((s) => s !== "email") : false;
    },
    processFallback: (localValue) => {
      if (localValue === void 0) {
...
var AppKitBaseClient = class {
  constructor(options) {
    ...
    this.setConnectedWalletInfo = (connectedWalletInfo, chain) => {
      const type2 = ProviderUtil.getProviderId(chain);
  ...
  createClients() {
    this.connectionControllerClient = {
      ...
      connectExternal: async ({ id, address, info, type: type2, provider, chain, caipNetwork, socialUri }) => {
        var _a, _b, _c, _d, _e2, _f;
        const activeChain = ChainController.state.activeChain;
  ...
  getAccount(_namespace) {
    ...
    const allAccounts = connections.flatMap((connection) => connection.accounts.map(({ address, type: type2 }) => CoreHelperUtil.createAccount(namespace, address, type2 || "eoa")));
    if (!accountState) {
      return void 0;
    }
    return {
...
var AppKit = class extends AppKitBaseClient {
  ...
  async injectModalUi() {
    if (!isInitialized && CoreHelperUtil.isClient()) {
      await import("./basic-NFCL2QIN.js");
      await import("./w3m-modal-67VSXDLF.js");
...
var UniversalConnector = class _UniversalConnector {
  constructor({ appKit, provider, config: config3 }) {
    this.appKit = appKit;
    this.provider = provider;
    this.config = config3;
  }
  static async init(config3) {
    const provider = await vi.init({
      projectId: config3.projectId,
      metadata: config3.metadata
    });
    const appKitConfig = {
      networks: config3.networks.flatMap((network) => network.chains),
      projectId: config3.projectId,
      metadata: config3.metadata,
    ...
    return new _UniversalConnector({ appKit, provider, config: config3 });
  }
  async connect() {
    var _a;
...
function ps(e2, t) {
  return e2 ? async (s, o, n) => {
    ...
    return r !== null && "txId" in r && r.txId && !("txid" in i) && (i.txid = r.txId), r !== null && "hex" in r && r.hex && typeof r.hex == "string" && !("psbt" in i) && (i.psbt = base64.encode(hexToBytes(r.hex))), i;
  } : t;
}
...
var Ns = (e2) => {
  var t;
  return { psbt: base64.encode(hexToBytes(e2.hex)), signInputs: typeof e2.signAtIndex == "number" ? [e2.signAtIndex] : e2.signAtIndex, allowedSighash: (t = e2.allowedSighash) == null ? void 0 : t.map((s) => Ae[s]) };
};
...
@noble/curves/esm/p256.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=chunk-KWTEGIWR.js.map
